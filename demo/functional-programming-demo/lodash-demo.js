...
// 由于这种执行的延后，因此lodash可以进行shortcut fusion这样的优化，
// 通过合并链式iteratee大大降低迭代的次数。从而大大提供其执行性能。
...
var names = _.chain(users)
	...
	.value();
console.log(names);
// 个人比较喜欢lodash延迟计算的现实value，以及JavaScript的函数式风格。
// 在这里首先将users对象包装成为lodash对象，再map获取所有用户的名称，并最后利用join将用户名称以”，“连接在一起。
// 注意这里只是一串方法链，如果你没有显样的调用value方法，使其立即执行的化，你将会得到如下的LodashWrapper延迟表达式：
// LodashWrapper {
// 	__wrapped__: LazyWrapper,
// 	__actions__: Array[1],
// 	__chain__: true,
// 	constructor: function,
// 	after: function…
// }
// 因为延迟表达式的存在，因此我们可以多次增加方法链，但这并不会被执行，所以不会存在性能的问题，
// 最后知道我们需要使用的时候，使用value显式立即执行即可。
var names = _.chain(users)
	.pluck('user')
	.join(", ")
	.value();
...
// 在这里博主多加了一个map作为log输出，
// 如果你执行这行代码的时候，你会惊奇的看见这里只会有一个user的输出，
// 这点可以证明在立即执行的时候lodash为我们的方法链做了可靠的优化；
// 如果我们去掉first函数你则会看见有3个user对象的输出。
...
var userObj = _.chain(users)
  ...
  .value();
console.log(userObj);
// 利用lodash首先将user数组map为[key, value]的数组集合，
// 最后利用zipObject将结果转换为Object对象，zipObject会利用结果集的第一项作为key，第二项作为value生产Object。
