...
// JavaScript是以函数为一等公民的函数式语言。
// 函数在JavaScript中也是一个对象（继承制Function），函数也可以作为参数传递成函数变量。
// 最近几年函数式也因为其无副作用的特性、透明性、惰性计算等在高并发，大数据领域火起来了。
...
// 首先上面会利用正则来获取传入函数的参数个数。
// 再返回一个函数的代理，每次的调用都会将传入参数缓存在args临时变量中，直到参数个数饱和才会立即执行。
// 代码比较冗长，慢慢品味，当然有不足之处，也希望大家指出来。
