...
function segment(hans) {
  ...
  // 词语拼音库。
  PHRASES_DICT = PHRASES_DICT || module["require"]("./node_modules/pinyin/src/phrases-dict");
  return jieba.cut(hans);
}
if (isNode) {
  // 拼音词库，node 版无需使用压缩合并的拼音库。
  PINYIN_DICT = module["require"]("./node_modules/pinyin/src/dict-zi");
} else {
  PINYIN_DICT = buildPinyinCache(require("./node_modules/pinyin/src/dict-zi-web"));
}
...
var PINYIN_STYLE = {
  ...
  RUBY: 5, // 汉字加拼音带音标 如： <ruby>汉<rt>hàn</rt></ruby>
};
// 带音标字符。
var PHONETIC_SYMBOL = require("./node_modules/pinyin/src/phonetic-symbol.js");
...
function single_pinyin(han, options) {
  ...
  if (!options.heteronym) {
    if (PINYIN_STYLE.RUBY == options.style) {
      return '<ruby>' + han + '<rt>' + pys[0] + '</rt></ruby>';
    } else {
...
pinyin.STYLE_RUBY = PINYIN_STYLE.RUBY;
